...
if ( !class_exists ( 'TWBanner' ) ) {
  require_once( WDFMInstance(2)->plugin_dir . '/banner_class.php' );
}
if ( WDFMInstance(2)->is_free ){
  $tw_banner_params = array(
    'menu_postfix' =>  WDFMInstance(2)->menu_postfix, // To display on only current plugin pages.
    'prefix' => WDFMInstance(2)->handle_prefix, // Current plugin prefix.
    'logo' => '/images/form-maker-logo.png', // Current plugin logo relative URL.
    'plugin_slug' => 'contact-form-maker', // Current plugin slug.
    'plugin_url' => WDFMInstance(2)->plugin_url, // Current plugin URL.
    'plugin_id' => 95, // Current plugin id.
    'text' => sprintf(__("%s Form Maker recommends:%s %sProtect your website from malware NOW for FREE with 10Web Security Service!%s", WDFMInstance(2)->prefix), '<span>','</span>', '<span>','</span>'), // Banner text.
